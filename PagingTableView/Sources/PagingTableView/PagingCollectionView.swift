...
/// A `UICollectionView` subclass that supports pull-to-refresh and load-more functionality using RxSwift and ESPullToRefresh.
open class PagingCollectionView: UICollectionView {
    
    /// The standard `UIRefreshControl` instance used for pull-to-refresh.
    private let _refreshControl = UIRefreshControl()
    
    /// A `Binder` that controls the refreshing state of the collection view.
    ///
    ...
    /// A `Binder` that controls the loading-more state of the collection view.
    ///
    ...
    /// Initializes the collection view from a nib file.
    ///
    ...
    /// Removes the custom `ESPullToRefresh` header from the collection view.
    open func removeRefreshHeader() {
        es.removeRefreshHeader()
    }
    
    /// Removes the custom `ESPullToRefresh` footer from the collection view.
